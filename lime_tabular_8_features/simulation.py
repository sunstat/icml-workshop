...
from decision_tree_label import DecisionTree
...
def determine_region_3d(dtree, x):

    return dtree.get_label(x)


def generate_data(sample_size, feature_size, coefs, determine_region_fn, decision_tree):
    ...
    for x in X:
        ...
        y = np.append(y,int(sum(x*coefs[determine_region_fn(decision_tree, x)])>=0))
        #print(determine_region_fn(dtree, x), y[-1])
...
def fit_blackmodel(train_X, train_y, test_X, test_y, blackmodel):
    ...
    if blackmodel == 0: 
        ...
        print("Test Accuracy:", metrics.accuracy_score(test_y, pred_y))
        return rf        
        ...
        print('Train Accuracy:', metrics.accuracy_score(train_y, gbc.predict(train_X)))
        print("Test Accuracy:", metrics.accuracy_score(test_y, pred_y))
        return gbc       
...
def test_generate_data():
    ...
    try:
        ...
        print('Train Accuracy:', metrics.accuracy_score(train_y, black_model.predict(train_X)))
        print("Test Accuracy:", metrics.accuracy_score(test_y, black_model.predict(test_X)))
    ...
    return train_X, train_y, test_X, test_y, black_model, coefs, dtree
